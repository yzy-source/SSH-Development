														...
														<div class="panel panel-red">
															...
															<div class="panel-body">
																<table class="table table-hover">
																	...
																	<tbody>
																		<c:forEach items="${requestScope.storePage.dataList}"
																			var="store" varStatus="status">

																			<tr>
																				<td>${status.count}</td>
																				<td>${store.products.productName}</td>
																				<td>${store.warehouse.WId}</td>
																				<td>${store.storeNum}</td>

																			</tr>
																		</c:forEach>
																		<tr>
																			<td>每页${requestScope.storePage.pageSize}条记录
																				总共${requestScope.storePage.totalPage}页</td>
																			<td>当前：第${requestScope.storePage.currentPage} /
																				${requestScope.storePage.totalPage}页</td>
																			<td>请选择：第 <c:forEach begin="1"
																					end="${requestScope.storePage.totalPage}" step="1"
																					var="i">
																					<c:if
																						test="${i==requestScope.storePage.currentPage}">
						【${i}】
					</c:if>
																					<c:if
																						test="${i!=requestScope.storePage.currentPage}">
